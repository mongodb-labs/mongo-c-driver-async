...
concept readable_stream = requires(T& strm, Bufs bufs) {
    { asio::async_read(strm, bufs, asio_as_nanosender) } -> nanosender_of<result<std::size_t>>;
};

template <typename T, typename Bufs = asio::const_buffer>
concept writable_stream = requires(T& strm, Bufs bufs) {
    { asio::async_write(strm, bufs, asio_as_nanosender) } -> nanosender_of<result<std::size_t>>;
};
